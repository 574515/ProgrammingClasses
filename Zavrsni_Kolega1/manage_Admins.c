...
void admin_Input(void) {
	...
	ADMIN* adminF = NULL;
	adminF = allocate_Admin(1);
	if (adminF == NULL) {
		perror("Greska strukture");
		exit(EXIT_FAILURE);
	}
	...
	if (!admin_File) {
		...
		check_Admin = allocate_Admin(admins_Number);
		if (check_Admin == NULL) {
			perror("Greska strukture");
			exit(EXIT_FAILURE);
		}
...
void admin_Delete(void) {
	...
	ADMIN* deleteS = NULL;
	deleteS = allocate_Admin(admins_Number);
	if (deleteS == NULL) {
		perror("Greska strukture");
		exit(EXIT_FAILURE);
	}
	...
	case 2: {
		char* del_Username;
		int found_Var = 0;
		del_Username = calloc(20, sizeof(char));
		if (!del_Username) {
			exit(EXIT_FAILURE);
		}
		c = getchar();
...
void admin_Search(void) {
	...
	search_S = allocate_Admin(admins_Number);
	if (search_S == NULL) {
		perror("Greska strukture");
		exit(EXIT_FAILURE);
	}
	...
	case 2: {
		char* search_FKIme = calloc(20, sizeof(char));
		if (!search_FKIme) {
			exit(EXIT_FAILURE);
		}
		system("cls");
...
void admin_List(void) {
	...
	list_S = allocate_Admin(admins_Number);
	if (list_S == NULL) {
		perror("Greska strukture");
		exit(EXIT_FAILURE);
	}
