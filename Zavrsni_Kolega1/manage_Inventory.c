...
void inventory_Input(void) {
	...
	INV* inventoryS = NULL;
	inventoryS = allocate_Inventory(1);
	if (inventoryS == NULL) {
	...
	if (!inventory_File) {
		...
		INV* check_Inv = NULL;
		check_Inv = allocate_Inventory(inventory_Number);
		if (check_Inv == NULL) {
			perror("Greska strukture");
			exit(EXIT_FAILURE);
		}
...
void inventory_Delete(void) {
	...
	fread(&inventory_Number, sizeof(int), 1, inventory_File);
	INV* deleteS = NULL;
	deleteS = allocate_Inventory(inventory_Number);
	if (deleteS == NULL) {
		perror("Greska strukture");
		exit(EXIT_FAILURE);
	}
...
void inventory_Search(void) {
	...
	search_S = allocate_Inventory(inventory_Number);
	if (search_S == NULL) {
		perror("Greska strukture");
		exit(EXIT_FAILURE);
	}
...
void inventory_List(void) {
	...
	INV* list_S = NULL;
	fread(&inventory_Number, sizeof(int), 1, inventory_File);
	list_S = allocate_Inventory(inventory_Number);
	if (list_S == NULL) {
		perror("Greska strukture");
		exit(EXIT_FAILURE);
	}
