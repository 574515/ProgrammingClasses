...
ADMIN* allocate_Admin(int admins_Number) {
	ADMIN* admin_Str= malloc(admins_Number * sizeof(ADMIN));
	if (!admin_Str) {
		return NULL;
	}
	else {
...
INV* allocate_Inventory(int inventory_Number) {
	INV* inv_Str = malloc(inventory_Number * sizeof(INV));
	if (!inv_Str) {
		return NULL;
	}
	else {
...
/*
obje funkcije su iste, različite strukture samo
predaje se broj koliko treba zauzeti (jer struktura nije primitivan tip podatka pa se množi sa sizeof(struktura)
if(!inv_Str) je isto što i if(inv_Str == NULL) -> i za admin ista stvar
dakle ako je null, vrati null, ako nije null, vrati tu zauzetu strukturu
e sad bi trebalo još napraviti provjere kod svakog poziva, što jesam, pa vidi jel ti radi
*/
